...
const Sponsors: React.FC = () => {
  ...
  const communityPartners = [
    { name: "TRYHACKME", logo: "https://tryhackme.com/img/favicon.png", url: "https://tryhackme.com" }
  ];

  const cybersecurityPartners = [
    { name: "JSMON", logo: "https://i.ibb.co/Kj2XP2Xn/dark-icon-1.png", url: "https://jsmon.sh" }
  ];
  ...
  return (
    <section className="space-y-12">
          ...
          <a key={idx} href={spon.url} target="_blank" rel="noopener noreferrer">
            <ComicPanel className="group hover:rotate-3 transition-transform p-0 !shadow-[8px_8px_0_0_rgba(0,0,0,1)] hover:!shadow-[12px_12px_0_0_rgba(0,0,0,1)]">
              <div className="aspect-square overflow-hidden relative bg-white flex items-center justify-center p-3">
                <img src={spon.logo} alt={spon.name} className="w-full h-full object-contain group-hover:scale-110 transition-transform duration-300" referrerPolicy="no-referrer" />
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
        {cybersecurityPartners.map((item, idx) => (
          <a key={idx} href={item.url} target="_blank" rel="noopener noreferrer">
            <ComicPanel className="group hover:rotate-3 transition-transform p-0 !shadow-[8px_8px_0_0_rgba(0,0,0,1)] hover:!shadow-[12px_12px_0_0_rgba(0,0,0,1)]">
              <div className="aspect-square overflow-hidden relative bg-black flex items-center justify-center p-4">
                ...
                <div className="absolute bottom-0 left-0 right-0 bg-black text-white text-center py-1 font-comic text-xl uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity border-t border-white">
                  {item.name}
                </div>
              </div>
            </ComicPanel>
      ...
      <div className="flex items-center gap-6">
        <h2 className="font-comic text-6xl uppercase px-8 py-2 bg-comic-yellow text-black -rotate-1 border-4 border-black">Community Partners</h2>
        <div className="h-4 bg-black flex-1 halftone-pattern" />
      </div>
